...
const LeadListScreen = ({ navigation }: any) => {
  ...
  const filteredLeads = leads.filter(lead => {
    const matchesSearch = lead.studentName.toLowerCase().includes(search.toLowerCase()) || lead.phone.includes(search);
...
const styles = StyleSheet.create({
  ...
  searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', margin: 15, paddingHorizontal: 15, borderRadius: 10, elevation: 2 },
  searchIcon: { marginRight: 10 },
  searchInput: { flex: 1, height: 45, fontSize: 16 },
  filterContainer: { flexDirection: 'row', paddingHorizontal: 15, marginBottom: 10 },
  filterChip: { paddingVertical: 6, paddingHorizontal: 15, borderRadius: 20, backgroundColor: '#e0e0e0', marginRight: 10 },
  activeChip: { backgroundColor: '#2196F3' },
  filterText: { color: '#666', fontSize: 14 },
  activeFilterText: { color: '#fff', fontWeight: 'bold' },
  listContainer: { padding: 15 },
  leadCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', padding: 15, borderRadius: 12, marginBottom: 12, elevation: 1 },
  leadInfo: { flex: 1 },
  ...
  statusBadge: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 5, marginRight: 10 },
